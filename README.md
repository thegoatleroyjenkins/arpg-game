...
- **Whiff Swing Commitment (3D prototype)** — Data-driven whiff behavior (`light_attack_allow_whiff_swing`, `light_attack_whiff_cooldown_multiplier`, `light_attack_whiff_stamina_multiplier`, `light_attack_whiff_lunge_multiplier`) now applies a partial attack commit even with no target hit, improving melee pacing clarity and reducing zero-commit spam
...
- **Light Attack Target Priority Weights (3D prototype)** — Data-driven target scoring now blends distance and forward alignment (`light_attack_targeting_distance_weight`, `light_attack_targeting_alignment_weight`) so swings prefer enemies centered in your attack cone instead of only nearest-first side targets
- **Light Attack Execute Priority Bias (3D prototype)** — Data-driven execute targeting bias (`light_attack_targeting_execute_bias_weight`) now lowers target-selection scores for enemies in execute range, making low-health finish opportunities more readable without hardcoded target overrides
...
- **Damage-Type Popup Readability Tags (3D prototype)** — Combat dummy damage popups now read resolver `damage_multiplier` results and display data-driven WEAK/RESIST tags + tinting (`weak_popup_*`, `resist_popup_*`, threshold exports), improving combat feedback clarity when tuning elemental/armor profiles
...
- **Light Attack Combo HUD Readout (3D prototype)** — Modular HUD now listens to a dedicated `light_attack_combo_changed` signal and shows live combo stacks + reset timer (`light_attack_combo_max_stacks`, `light_attack_combo_reset_time`), improving melee momentum readability without coupling HUD logic to combat internals
...
- **Light Attack Auto-Facing Assist (3D prototype)** — Data-driven melee facing assist now rotates the player partway toward the primary in-range target on attack (`light_attack_auto_face_enabled`, max angle, max turn per swing), improving close-quarters strike consistency without hardcoded aim snaps
...
- **Light Attack Execute Window (3D prototype)** — Data-driven finisher tuning now grants bonus light-attack damage against low-health combat actors (`light_attack_execute_enabled`, `light_attack_execute_health_ratio_threshold`, `light_attack_execute_damage_multiplier`), rewarding clean target focus and improving end-of-fight pacing clarity
- **Light Attack Combo Ramp (3D prototype)** — Data-driven combo tuning now increases light-attack damage when consecutive hits land inside a reset window (`light_attack_combo_enabled`, `light_attack_combo_reset_time`, `light_attack_combo_damage_per_stack`, `light_attack_combo_max_stacks`), adding clearer melee momentum and reward for sustained close-range pressure
- **Critical Hit Camera Impulse Tuning (3D prototype)** — Critical hit camera kick bonuses are now authored in `PlayerTuning3D` (`light_attack_crit_camera_impulse_strength_bonus`, `light_attack_crit_camera_impulse_vertical_bonus`) so combat feel can be tuned per profile/resource without hardcoded player-node exports
- **Light Attack On-Hit Stamina Refund (3D prototype)** — Data-driven melee sustain tuning (`light_attack_hit_stamina_restore`, `light_attack_crit_stamina_restore_bonus`) now restores stamina on successful light-attack hits (with extra refund on crits), improving offensive tempo and reducing dead-time after clean engagements without hardcoded stamina grants
...
- **Combat Dummy Popup De-Stacking (3D prototype)** — Data-driven popup spread + vertical stagger tuning (`damage_popup_spread_radius`, `damage_popup_vertical_stagger`) now offsets simultaneous hit numbers in world space, reducing text overlap during fast multi-hit windows without hardcoded layout logic
...
- ✅ Added data-driven light-attack execute tuning (`light_attack_execute_enabled`, `light_attack_execute_health_ratio_threshold`, `light_attack_execute_damage_multiplier`) so low-health enemies take bonus finisher damage, improving target-focus decisions and end-of-fight combat pacing without hardcoded enemy branches
